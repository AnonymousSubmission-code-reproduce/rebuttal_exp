...
class AugmentedMemory_Optimizer(BaseOptimizer):
    ...
    def _optimize(self, oracle, config):
        ...
        while True:
            ...
            for _ in range(model.augmentation_rounds):
                ...
                randomized_smiles_list = get_randomized_smiles(self.args.oracles[0], self.args.mol_lm, self.mol_lm, self.args.bin_size, smiles, model.prior)
    ...
    @staticmethod
    def add_experience_replay(model, loss, agent_likelihood, prior_likelihood, smiles, scores, override, args, mol_lm):
        ...
        if model.augmented_memory and not override:
            if len(model.replay_buffer.buffer) == 0:
                ...
                exp_smiles, exp_scores, exp_prior_likelihood = model.replay_buffer.augmented_memory_replay(args, mol_lm)
