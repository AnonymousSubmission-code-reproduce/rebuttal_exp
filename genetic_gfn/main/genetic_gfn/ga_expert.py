...
def reproduce_LLM(mating_pool, mutation_rate, lm_name, mol_lm):
    """
    Args:
        mating_pool: list of RDKit Mol
        mutation_rate: rate of mutation
        ...
        try:
            ...
            if editted_mol != None:
                ...
                if s != None:
                    ...
                    return editted_mol, parent_a, parent_b
        except:
            print('Failed to edit the molecule!!!')
            return None, None, None
    elif lm_name == 'GPT-4':
        try:
            editted_mol = mol_lm.edit(parent_a, parent_b, mutation_rate)
            if editted_mol != None:
                s = Chem.MolToSmiles(editted_mol)
                if s != None:
                    print("adding editted molecule!!!")
...
class GeneticOperatorHandler:
    ...
    def query(self, query_size, mating_pool, pool, lm_name, mol_lm, bin_size, rank_coefficient=0.01, mutation_rate=None, return_dist=False):
        # print(mating_pool)
        ...
        offspring_tuple = [reproduce(cross_mating_pool, mutation_rate) for _ in range(query_size)]
        edited_mols = []
        if lm_name == 'BioT5':

            while len(offspring_tuple) < bin_size:
                try:
                    editted_mol, pa, pb = reproduce_LLM(cross_mating_pool, mutation_rate, lm_name, mol_lm)
                    #editted_mol = reproduce_LLM(mol_lm.edit([m])[0])

                    if editted_mol != None:
                        s = Chem.MolToSmiles(editted_mol)
                        if s != None:
                            print("adding editted molecule!!!")
                            offspring_tuple.append((editted_mol, pa, pb))
                except:
                    continue
        elif lm_name == 'GPT-4':
            while len(offspring_tuple) < bin_size:
        ...
        offspring_mol, parents_a, parents_b = zip(*offspring_tuple)
