...
from main.genetic_gfn.genetic_operator.biot5 import BioT5
from main.genetic_gfn.genetic_operator.GPT4 import GPT4
            ...
            if config['population_size'] and len(self.oracle) > config['population_size']:
                ...
                for g in range(config['ga_generations']):
                    ...
                    child_smis, child_n_atoms, _, _, ga_results = ga_handler.query(
                            query_size=config['offspring_size'], mating_pool=mating_pool, pool=pool, lm_name = self.args.mol_lm, mol_lm = self.mol_lm, bin_size = self.args.bin_size, 
